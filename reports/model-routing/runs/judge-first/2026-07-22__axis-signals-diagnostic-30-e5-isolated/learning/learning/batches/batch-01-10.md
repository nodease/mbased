# Judge-first 자동 모델 라우팅 10회 학습 검증 보고서

## 한눈에 보는 결론

이 단계는 자동 라우팅만 10회 실행해 첫 50건으로 학습하고 다음 50건으로 일반화 성능과 학습 버전 발행 여부를 확인합니다.

- 자동 라우팅 총 제품 비용: $0.032960
- 고가 고정 비교: 이번 실행에서 제외
- 중간 고정 비교: 이번 실행에서 제외
- 저가 고정 비교: 이번 실행에서 제외
- 자동 라우팅에서 Runtime Judge가 실제 호출된 횟수: 10/10

자동 라우팅 비용에는 요청 처리 모델 비용과 Runtime Judge 비용을 모두 포함했습니다. 실험의 품질 평가 Judge 비용은 제품 기능의 런타임 비용이 아니므로 별도로 표시합니다.

## 실험 조건

- 실행 시각: 2026-07-21T21:18:39.073041+00:00
...
- 정책 refresh: 100회. 10회 실험 동안 정책 교체를 막고, Judge label을 누적한 local router의 전환만 측정했습니다.

## 데이터셋

이 단계의 10개 요청은 여러 업무 유형과 난이도를 섞었고, 동일 문장을 반복하지 않았습니다.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 자동 라우팅 | $0.013977 | $0.018983 | $0.032960 | 10225ms | 10450ms | 미평가 | 100.0% | - |
...
- `gpt-5.4-mini`: 1회
- 사전 가설 기준 적중: 6/10건
- 성능 부족 선택: 4건 / 과도한 선택: 0건 / 미분류: 0건

## 로컬 라우터 학습 수준

- Judge label 누적: 10건
- 실제 선택 모델 종류: 5개 ({'gpt-4o-mini': 4, 'gpt-5-mini': 3, 'gpt-5.4-mini': 1, 'gpt-4.1': 1, 'gpt-4.1-mini': 1})
- 처리 출처 분포: {'runtime_judge': 10}
- 로컬 라우터가 Judge 없이 직접 선택한 횟수: 0회
- 로컬 라우터가 자신 없어 Judge로 되돌린 횟수: 0회
- Judge 신뢰도 평균 / P95: 0.918 / 0.970
- 로컬 takeover 최소 신뢰도: 0.78
- 최종 저장 정책 학습 모드: judge_first
- 최종 저장 학습 표본: Judge 9건 / 운영 완료 9건
- 학습 artifact: multilingual_e5_task_requirements_ordinal_v3 (intfloat/multilingual-e5-base)
- artifact 학습 예시 수: 9건 / artifact 모델 label: ['gpt-4.1-mini', 'gpt-4o-mini', 'gpt-5-mini', 'gpt-5.4-mini']
