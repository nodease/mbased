# Judge-first 자동 모델 라우팅 1회 경제성 비교 보고서

## 한눈에 보는 결론

이 단계는 학습에 쓰지 않은 1개 요청을 자동 라우팅, 중간 모델 고정, 고가 모델 고정에 똑같이 보내 비용·속도·품질을 비교합니다.

- 자동 라우팅 총 제품 비용: $0.002026
- 고가 고정 대비 자동 라우팅 순절감: $0.010464 (절감)
- 중간 고정 비교: 이번 실행에서 제외
- 저가 고정 비교: 이번 실행에서 제외
- 자동 라우팅에서 Runtime Judge가 실제 호출된 횟수: 1/1

자동 라우팅 비용에는 요청 처리 모델 비용과 Runtime Judge 비용을 모두 포함했습니다. 실험의 품질 평가 Judge 비용은 제품 기능의 런타임 비용이 아니므로 별도로 표시합니다.

## 실험 조건

- 실행 시각: 2026-07-21T22:44:52.869030+00:00
...
- 정책 refresh: 100회. 1회 실험 동안 정책 교체를 막고, Judge label을 누적한 local router의 전환만 측정했습니다.

## 데이터셋

이 단계의 1개 요청은 여러 업무 유형과 난이도를 섞었고, 동일 문장을 반복하지 않았습니다.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 자동 라우팅 | $0.000163 | $0.001863 | $0.002026 | 5174ms | 5372ms | 62.0 | 100.0% | 0.0% |
| 고가 고정 (gpt-5.6-sol) | $0.012490 | $0.000000 | $0.012490 | 7304ms | 7543ms | 78.0 | 100.0% | 0.0% |

## 자동 라우팅이 실제로 고른 모델

- `gpt-4o-mini`: 1회
- 사전 가설 기준 적중: 1/1건
- 성능 부족 선택: 0건 / 과도한 선택: 0건 / 미분류: 0건

## 로컬 라우터 학습 수준

- Judge label 누적: 1건
- 실제 선택 모델 종류: 1개 ({'gpt-4o-mini': 1})
- 처리 출처 분포: {'runtime_judge': 1}
- 로컬 라우터가 Judge 없이 직접 선택한 횟수: 0회
- 로컬 라우터가 자신 없어 Judge로 되돌린 횟수: 0회
- Judge 신뢰도 평균 / P95: 0.930 / 0.930
...
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | holdout_product_help | economy | gpt-4o-mini | 62.0 | $0.002026 | 78.0 | $0.012490 |
