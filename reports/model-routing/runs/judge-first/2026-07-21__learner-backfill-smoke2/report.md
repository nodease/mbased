# Judge-first 자동 모델 라우팅 1회 경제성 실험 보고서

## 한눈에 보는 결론

이 실험은 같은 기업 요청 처리 워크플로우를 1개의 서로 다른 요청으로 실행해, 자동 모델 라우팅이 비싼 모델만 고정하는 경우보다 돈을 아끼는지, 싼 모델만 고정하는 경우보다 결과 품질을 지키는지 확인한 결과입니다.

- 자동 라우팅 총 제품 비용: $0.002716
- 고가 고정 비교: 이번 실행에서 제외
- 중간 고정 비교: 이번 실행에서 제외
- 저가 고정 비교: 이번 실행에서 제외
...
- 실행 시각: 2026-07-21T14:19:16.002598+00:00
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 자동 라우팅 | $0.001517 | $0.001199 | $0.002716 | 27689ms | 27907ms | 86.0 | 100.0% | 100.0% |

## 자동 라우팅이 실제로 고른 모델

- `gpt-5.4-mini-[REDACTED]`: 1회
- 사전 가설 기준 적중: 0/1건
- 성능 부족 선택: 0건 / 과도한 선택: 0건 / 미분류: 1건

## 로컬 라우터 학습 수준

- Judge label 누적: 1건
- 실제 선택 모델 종류: 1개 ({'gpt-5.4-mini-[REDACTED]': 1})
- 처리 출처 분포: {'runtime_judge': 1}
- 로컬 라우터가 Judge 없이 직접 선택한 횟수: 0회
- 로컬 라우터가 자신 없어 Judge로 되돌린 횟수: 0회
- Judge 신뢰도 평균 / P95: 0.880 / 0.880
...
| ---: | --- | --- | --- | ---: | ---: |
| 1 | finance_closing_approval | advanced | gpt-5.4-mini-[REDACTED] | 86.0 | $0.002716 |
