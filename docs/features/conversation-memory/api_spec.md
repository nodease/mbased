...
Public history contract는 Conversation bearer token, If-Match와 lifecycle Idempotency-Key를 사용하지 않는다. Authenticated internal session mutation의 status/idempotency 계약은 후속 API activation 전에 확정한다.
