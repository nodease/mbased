...
프론트 UI/UX 작업 중 백엔드 실행 환경에 덜 의존하도록 mock 모드를 추가했다.

- `?mock=1` 또는 mock route에서 workflow API 응답을 mock 데이터로 대체한다.
...
- 번호는 workflow 노드에만 부여한다.
- `note` 노드는 번호 발급 대상에서 제외한다.
