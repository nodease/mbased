...
- Gateway는 외부 `X-Request-ID` 중 canonical RFC 4122 UUID만 보존하고 다른 값은 새 UUID로 대체한다. CSRF token은 ASCII 형식을 equality 이전에 검증하며, CSRF 거부의 동기 audit/metric persistence는 전용 bounded worker thread에서 수행해 event loop와 raw request ID를 보호한다.
